...
import { GameMeta, IBoardRepository, IGameRepository, IMoveRepository, IPieceRepository, IPlayerRepository } from "./ports";
...
import { GamePiece } from "../core/models/GamePiece";
  ...
  // naive piece id helper (since domain doesn't have ids on GamePiece)
  const idOf = (p: object) => {
    // @ts-ignore
    if (!p.__id) p.__id = Math.random().toString(36).slice(2);
    // @ts-ignore
    return p.__id as string;
  };
    ...
    async saveInitialPieces(gameId: string, ps: Player[]): Promise<void> {
      ps.forEach((pl) =>
        pl.getAvailablePieces().forEach((gp) => {
          pieces.set(idOf(gp), { gameId, piece: gp });
        })
      );
    }
